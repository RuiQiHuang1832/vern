...
export default function MediaSelection(props: MediaSelectionProps) {
  ...
  const handleSearch = () => {
    ...
    (async () => {
      try {
        ...
        const searchResponse = await fetch(`https://api.themoviedb.org/3/search/movie?query=${searchData["mediaName"]}&include_adult=false&language=en-US&page=1&year=${startDate?.getFullYear()}`, options).then((response) => response.json());
        ...
        const [movieDetailsResponse, videosResponse] = await Promise.all([fetch(`https://api.themoviedb.org/3/movie/${movieId}`, options).then((response) => response.json()), fetch(`https://api.themoviedb.org/3/movie/${movieId}/videos`, options).then((response) => response.json())]);
  ...
  const handleAdd = () => {
    const currentLength = props.selectedTitles.length;

    if (props.selectedTitles[currentLength - 1].isPlaceholder && searchData.mediaName !== "") {
      ...
      if (!isDuplicate) {
        ...
        const updatedTitles = [...props.selectedTitles];

        // Try to replace the first `placeHolder` if it exists
        const firstPlaceholderIndex = updatedTitles.findIndex((item) => item?.isPlaceholder === true);
        if (firstPlaceholderIndex !== -1) {
          updatedTitles[firstPlaceholderIndex] = newData;
        } else {
          updatedTitles.push(newData);
        }

        props.onTitleAdd(updatedTitles);
  ...
  return (
    <div className={`${props.currentPage > 2 ? "d-none" : ""}`}>
      <div className="input-group mb-3">
        ...
        <Button buttonColor={{ cssColor: "white" }} eventOnClick={handleSearch} type="submit" width="auto" radius="0px" padding="10px" styleClass={`${styles["nav-search"]} btn ${styles["search-radius"]}`}>
      ...
      <div className="text-center mb-5 mt-4">
        {titleData?.mediaPoster == undefined || titleData?.isPlaceholder ? (
          ""
        ) : loader ? (
      ...
      <div className="d-flex justify-content-between align-items-center">
        <h4 className="mb-2 fw-light">Tray</h4>
        <button
          onClick={() => {
            if (props.selectedTitles[0]?.isPlaceholder) return;
            props.onClear([]);
          }}
          type="button"
          className="btn btn-danger d-flex align-items-center text-center p-2 green-red"
        >
          <FaTrashCan />
        </button>
      </div>
