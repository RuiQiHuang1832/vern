...
export default function HowItWorks() {
  return (
    <section style={{ paddingBottom: "15rem", background: "#1A1A1A" }} className={`d-flex flex-column justify-content-center `}>
      ...
      <div className={`${styles['parent-container']} col-10 mx-auto  align-items-center`}>
        {stepData.map((data, i) => (
 <FadeInElement key={data.id} value="fade-in-section">
          <div className={`${styles["steps-container"]} mt-3 align-items-center d-flex position-relative flex-wrap-reverse`}>
            <div style={{ gap: "2.5em", flex: "1 1 0px" }} className="d-flex justify-content-center">
                <Image priority src={data.icon} width={65} height={65} alt={data.id} className={`mb-4 item d-none d-xl-block`}></Image>
                <div style={{ maxWidth: "30em", }} className={`item mt-auto`}>
